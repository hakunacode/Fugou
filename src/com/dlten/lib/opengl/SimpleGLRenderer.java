 ...
 */
public class SimpleGLRenderer implements CMyGLDCView.Renderer {
    // Specifies the format our textures should be converted to upon load.
	private CMyGLDCView	m_parent = null;
...
//			changeSprites(gl);
    ...
    public void sizeChanged(GL10 gl, int width, int height) {
    	...
    	int	nDblOffset = CDCView.getDblOffsetY();
    	
//    	gl.glViewport(0, 0, width, height);
    	gl.glViewport(0, (int)(height-bufHeight)-nDblOffset, resWidth, bufHeight);	// modified by hrh 2011-0524
...
//        gl.glOrthof(0.0f, width, 0.0f, height, 0.0f, 1.0f);
        gl.glOrthof(0.0f, resWidth, 0.0f, bufHeight, 0.0f, 1.0f);	// modified by hrh 2011-0524
     ...
     * 2D texture maps. 
     */
    protected int loadBitmap(Context context, GL10 gl, Bitmap bitmap) {
        int textureName = -1;
...
//          gl.glTexParameterf(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_WRAP_T, GL10.GL_CLAMP_TO_EDGE);
    ...
    protected int loadBitmap(Context context, GL10 gl, int resourceId) {
        ...
        textureName = loadBitmap(context, gl, bitmap);
    ...
    protected void changeBitmap(GL10 gl, int textureName, Bitmap bitmap) {
        if (gl != null && textureName != -1 && bitmap != null) {
            gl.glBindTexture(GL10.GL_TEXTURE_2D, textureName);
            // GLUtils.texImage2D(GL10.GL_TEXTURE_2D, 0, bitmap, 0);
            // gl.glTexImage2D(GL10.GL_TEXTURE_2D, 0, internalformat, width, height, border, format, type, pixels)
            gl.glCopyTexImage2D(GL10.GL_TEXTURE_2D, 0, GL10.GL_RGBA, 0, 0, bitmap.getWidth(), bitmap.getHeight(), 0);
        }
    }
    ...
    private void createSprites(GL10 gl) {
        ...
        for (int x = 0; x < mSprites.length; x++) {
            ...
            // if (resource != lastLoadedResource)
            {
            	...
            	Bitmap bitmap = mSprites[x].getBmp();
            	// STD.logout("CMyGLDCView.createSprites()	start");
            	synchronized (bitmap)
            	{
            		lastTextureId = loadBitmap(mContext, gl, bitmap);
            	}
                lastLoadedResource = resource;
                // STD.logout("CMyGLDCView.createSprites()	end");
            }
    ...
    private void changeSprites(GL10 gl) {
        ...
        for (int x = 0; x < mSprites.length; x++) {
            lastTextureId = mSprites[x].getTextureName();
            if (lastTextureId != -1)
            {
            	Bitmap bitmap = mSprites[x].getBmp();
            	synchronized (bitmap)
