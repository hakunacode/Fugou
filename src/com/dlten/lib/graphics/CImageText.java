...
public class CImageText extends CImage {
    ...
    public CImageText(String text) {
    	...
    	CSize	dimensions = calculateTextSize(text, fontname, fontSize);
    	int		alignment = ANCHOR_TOP | ANCHOR_CENTER;
    	
    	createTexture(text, dimensions, alignment, fontname, fontSize);
    }
    ...
    private CSize calculateTextSize(String text, String fontname, float fontSize) {
        ...
        return CSize.make(measuredTextWidth, ascent + descent);
        
//    	int		maxlines = 200;
//        CDCView dc = CImage.getDCView();
//    	int		lineCount[] = new int[1];
//    	String[] strArray = new String[maxlines];
//    	return dc.calculateTextSize(dc.m_font, text, SC_RES_WIDTH, SC_RES_HEIGHT, 0, maxlines, strArray, lineCount);
    }
...
//					config = Bitmap.Config.ARGB_4444;
    ...
    public void draw( float fPosX, float fPosY ) {
    	float	x = fPosX;
    	float	y = fPosY;
    	float	w = m_totalWidth;
    	float	h = m_totalHeight;
		drawImage(m_textureID, x, y, w, h, 255);
    }
